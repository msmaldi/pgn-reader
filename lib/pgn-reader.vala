public errordomain GChess.PGNReaderError
{
    LOAD_ERROR
}

private enum GChess.State
{
    SYMBOL,
    MOVE_TEXT,
    TAGS,
...
public class GChess.PGNReader
{
    public const string RESULT_IN_PROGRESS = "*";
    public const string RESULT_DRAW = "1/2-1/2";
    public const string RESULT_WHITE = "1-0";
    public const string RESULT_BLACK = "0-1";
        ...
        while ((size = stream.read (buffer)) > 0)
        {
            for (long offset = 0; offset < size; offset++)
            {
                ...
                case State.SYMBOL:
                    ...
                    {
                        if (buffer_overflow (symbol_buffer, symbol_ptr, 255))
                            state = State.ERROR;
                        else
                            *symbol_ptr++ = c;
                    }
    ...
    static inline bool buffer_overflow (char *buffer, char *ptr, int size)
    {
